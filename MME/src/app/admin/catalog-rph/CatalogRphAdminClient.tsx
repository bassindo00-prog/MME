...
import { CatalogRph } from "@prisma/client";
import { Plus, Search, Edit2, Trash2, Image as ImageIcon, ExternalLink, RefreshCw, X, Upload } from "lucide-react";
import { addCatalogRph, updateCatalogRph, deleteCatalogRph } from "@/app/actions/catalogRph";

export function CatalogRphAdminClient({ initialData }: { initialData: CatalogRph[] }) {
  const [data, setData] = useState(initialData);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<CatalogRph | null>(null);
  const [formData, setFormData] = useState({ title: "", singer: "", composer: "", youtubeUrl: "", coverUrl: "" });
  ...
  const handleOpenModal = (item?: CatalogRph) => {
    if (item) {
      setEditingItem(item);
      setFormData({
        title: item.title,
      ...
      setEditingItem(null);
      setFormData({ title: "", singer: "", composer: "", youtubeUrl: "", coverUrl: "" });
    }
    setCoverFile(null);
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        const updated = await updateCatalogRph(editingItem.id, {
        ...
        const added = await addCatalogRph({
          title: formData.title,
          singer: formData.singer,
          composer: formData.composer,
          youtubeUrl: formData.youtubeUrl || null,
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Katalog RPH</h1>
          <p className="text-gray-500 mt-1">Manage catalog entries, covers, and YouTube links</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="relative mb-6">
          ...
          <input
            type="text"
            placeholder="Search by title, singer, or composer..."
            value={search}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="py-3 px-4 text-xs font-bold text-gray-500 uppercase">Singer</th>
            ...
            <tbody>
              {displayedData.map((item) => (
                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-600">{item.singer}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[70vh]">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Singer</label>
                  <input required type="text" value={formData.singer} onChange={e => setFormData({...formData, singer: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition" />
                </div>
                <div>
