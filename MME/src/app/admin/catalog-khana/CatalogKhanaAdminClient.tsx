...
import { CatalogKhana } from "@prisma/client";
import { Plus, Search, Edit2, Trash2, Image as ImageIcon, ExternalLink, RefreshCw, X, Upload } from "lucide-react";
import { addCatalogKhana, updateCatalogKhana, deleteCatalogKhana } from "@/app/actions/catalogKhana";

export function CatalogKhanaAdminClient({ initialData }: { initialData: CatalogKhana[] }) {
  const [data, setData] = useState(initialData);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<CatalogKhana | null>(null);
  ...
  const handleOpenModal = (item?: CatalogKhana) => {
    if (item) {
      setEditingItem(item);
      setFormData({
        songId: item.songId || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        const updated = await updateCatalogKhana(editingItem.id, payload);
        setData(prev => prev.map(d => d.id === updated.id ? updated : d));
      } else {
        const added = await addCatalogKhana(payload);
        setData([added, ...data]);
      }
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Katalog Khana</h1>
          <p className="text-gray-500 mt-1">Manage PT Khana Media Nusantara catalog</p>
        </div>
        <button 
